...
# coding: utf-8
"""
GC-MPNN - Active Learning via Evidential Regression Uncertainty
===============================================================

Workflow
--------
1. Load the evidential regression predictions CSV for the target gas (e.g. CO2).
   This CSV contains y_true, epistemic_std (and aleatoric_std, total_std) for
   every test polymer.

2. Select N_ACTIVE samples from the test set using one of two strategies:
     'uncertainty' - top-N by epistemic_std  (highest model uncertainty first)
     'random'      - uniformly random baseline (fixed seed for fairness)

   NOTE: epistemic_std is used rather than total_std because active learning
   targets model ignorance (reducible by new labels), not irreducible noise.

3. Match selected samples back via the 'smiles' column in the evidential CSV.
...
EVIDENTIAL_CSV = f'{EXPERIMENT_NAME}_evidential_test_{TEST_GAS}_predictions.csv'
...
def select_active_samples(evidential_csv_path: str,
                           n_active: int,
                           strategy: str,
                           random_seed: int = 42):
    """
    Load the evidential regression predictions CSV and select N_ACTIVE samples.

    For 'uncertainty' strategy, samples are ranked by epistemic_std (model
    uncertainty), which is the appropriate signal for active learning -
    aleatoric_std reflects irreducible noise and should not drive selection.

    Returns
    -------
    selected_df  : DataFrame of the N_ACTIVE chosen rows
    remaining_df : DataFrame of the un-chosen rows (new test set)
    """
    df = pd.read_csv(evidential_csv_path)
    n_total = len(df)
    ...
    if strategy == 'uncertainty':
        # Sort descending by epistemic_std; top N_ACTIVE are most uncertain
        sorted_df    = df.sort_values('epistemic_std', ascending=False).reset_index(drop=True)
    ...
    if strategy == 'uncertainty':
        print(f'  epistemic_std range of selected : '
              f'[{selected_df["epistemic_std"].min():.4f}, {selected_df["epistemic_std"].max():.4f}]')
        print(f'  epistemic_std range of remaining: '
              f'[{remaining_df["epistemic_std"].min():.4f}, {remaining_df["epistemic_std"].max():.4f}]')
...
def df_rows_to_graphs(df: pd.DataFrame,
    ...
    the smiles column in the evidential CSV is the ground truth identifier.
...
def preflight_smiles_audit(evidential_csv_path: str):
    """
    Verify that the evidential CSV has a 'smiles' column and that every
    entry is a non-null, non-empty string that RDKit can parse.

    Prints a summary and raises RuntimeError on critical failures.
    """
    ens_df  = pd.read_csv(evidential_csv_path)
    n_total = len(ens_df)

    if 'smiles' not in ens_df.columns:
        raise RuntimeError(
            f"No 'smiles' column found in {evidential_csv_path}.\n"
            f"Re-run the evidential regression script (updated version) which "
            f"writes the smiles column into the CSV.")
    ...
    print(f'  Evidential CSV    : {evidential_csv_path}')
...
def main():
    print('\n' + '='*80)
    print('GC-MPNN  ACTIVE LEARNING  [Evidential Regression Uncertainty]')
    print(f'Experiment       : {EXPERIMENT_NAME}')
    print(f'Test gas         : {TEST_GAS}')
    print(f'N_ACTIVE         : {N_ACTIVE}')
    print(f'Strategy         : {SELECTION_STRATEGY}')
    print(f'Evidential CSV   : {EVIDENTIAL_CSV}')
    print('='*80)
    ...
    preflight_smiles_audit(EVIDENTIAL_CSV)
    ...
    # Select active samples from evidential predictions CSV
    print(f'\nLoading evidential predictions: {EVIDENTIAL_CSV}')
    selected_df, remaining_df = select_active_samples(
        evidential_csv_path = EVIDENTIAL_CSV,
        n_active            = N_ACTIVE,
        strategy            = SELECTION_STRATEGY,
        random_seed         = RANDOM_SEED,
    )

    # Build graphs directly from smiles column in evidential CSV
    print('\nBuilding graphs from evidential CSV smiles column...')
    ...
    # Carry over epistemic_std of remaining samples for downstream analysis
    # (useful to verify that uncertainty-selected samples were genuinely harder)
    if 'epistemic_std' in remaining_df.columns:
        std_lookup = dict(zip(remaining_df['y_true'].values,
                               remaining_df['epistemic_std'].values))
        pred_df['epistemic_std_evidential'] = [std_lookup.get(v, np.nan) for v in y_true]
